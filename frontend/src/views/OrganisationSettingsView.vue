...
import { Key, Lock, Pencil, Plus, Save, Shield, Trash2 } from '@lucide/vue';
...
const fetchApiTokens = async () => {
    ...
    try {
        apiTokens.value = await api.organisations.tokens.list(orgId.value);
    } catch (err) {
...
const handleCreateToken = async (
    ...
    try {
        const res = await api.organisations.tokens.create(orgId.value, {
            name: tokenName,
            permissions,
            expiresAt
        });
...
const handleDeleteToken = async (token: ApiTokenDTO) => {
    if (
        ...
        return;
    try {
        await api.organisations.tokens.delete(orgId.value, token.id);
...
<template>
    <div class="p-4 lg:p-8">
        ...
        <div v-else-if="activeTab === 'rbac'">
            <div class="flex items-start justify-between mb-6 gap-4">
                <p class="text-sm text-gray-600 max-w-lg">
                    Define roles and their permissions. Roles control what
                    members can do within this organisation.
                </p>
                <button
                    class="shrink-0 px-4 py-2 bg-gray-900 text-white rounded text-sm hover:bg-gray-800 flex items-center gap-2 transition-colors"
                    @click="openCreate"
