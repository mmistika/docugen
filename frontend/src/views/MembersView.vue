...
<template>
    <div class="p-4 lg:p-8">
        <TabHeader title="Members">
            <template #actions>
                <button
                    class="px-4 py-2 bg-gray-900 text-white rounded text-sm hover:bg-gray-800 flex items-center gap-2 justify-center transition-colors"
        ...
        >
            <template #filters>
                <select
                    ...
                    class="px-4 py-2 border border-gray-300 rounded text-sm bg-white outline-none focus:ring-2 focus:ring-gray-300"
                >
        ...
        <DataTable :headers="tableHeaders" :items="filteredMembers">
            ...
            <template #cell:actions="{ item }">
                <button
                    class="p-1.5 hover:bg-gray-200 rounded transition-colors flex items-center gap-1.5 text-sm text-gray-600 cursor-pointer"
                    title="Manage roles"
            ...
            <template #mobile-item="{ item }">
                ...
                <div class="pt-3 border-t border-gray-100">
                    <button
                        class="w-full px-3 py-1.5 border border-gray-300 rounded text-sm hover:bg-gray-50 flex items-center justify-center gap-1.5 transition-colors"
                        @click="openManageRoles(item)"
                    >
                        <UserCog :size="15" />
                        Manage Roles
