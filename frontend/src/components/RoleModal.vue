...
const groupSomeSelected = (perms: PermissionDTO[]) =>
    perms.some((p) => selectedPerms.value.has(p.name)) && !groupAllSelected(perms)
...
<template>
  <Modal :show="show" :title="isEditMode ? 'Edit Role' : 'Create Role'" @close="emit('close')">
    <template #body>
      <div class="space-y-5">
        <div>
          ...
          <input
              ...
              : isEditMode
                ...
                : 'border-gray-300'"
          />
          <p v-if="isEditMode" class="mt-1 text-xs text-gray-400">Role name cannot be changed after creation.</p>
          <p v-else-if="nameError" class="mt-1 text-xs text-red-500">{{ nameError }}</p>
        </div>
        <div>
          ...
          <div class="space-y-3 max-h-72 overflow-y-auto pr-1">
            ...
            >
              <button
                  ...
                  class="w-full flex items-center gap-2 px-3 py-2 bg-gray-50 hover:bg-gray-100
                       transition-colors text-left"
              >
                <div
                    class="w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 transition-colors"
                    :class="groupAllSelected(perms)
                    ? 'bg-gray-900 border-gray-900'
                    : groupSomeSelected(perms)
                      ? 'bg-gray-400 border-gray-400'
                      : 'border-gray-300'"
                >
                  <svg v-if="groupAllSelected(perms)" class="w-2 h-2 text-white" fill="none" viewBox="0 0 10 8">
                    <path d="M1 4l3 3 5-6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                  </svg>
                  <div v-else-if="groupSomeSelected(perms)" class="w-1.5 h-0.5 bg-white rounded" />
                </div>
                <span class="text-xs font-semibold text-gray-700">{{ formatGroup(group) }}</span>
                <span class="ml-auto text-xs text-gray-400">
                  {{ perms.filter(p => selectedPerms.has(p.name)).length }}/{{ perms.length }}
                </span>
              </button>
              <div class="divide-y divide-gray-100">
                ...
                >
                  <div
                      class="w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 transition-colors"
                      :class="selectedPerms.has(perm.name)
                      ? 'bg-gray-900 border-gray-900'
                      : 'border-gray-300'"
                      @click.prevent="toggle(perm.name)"
                  >
                    <svg v-if="selectedPerms.has(perm.name)" class="w-2 h-2 text-white" fill="none" viewBox="0 0 10 8">
                      <path d="M1 4l3 3 5-6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                  </div>
                  <input
                      ...
                      class="sr-only"
                  />
                  <span class="text-xs text-gray-700">{{ formatPerm(perm.name) }}</span>
                </label>
              </div>
            </div>
          </div>
