...
const extraProperties = computed(() =>
    props.activeField
        ? (FIELD_TYPE_MAP[props.activeField.type]?.properties ?? [])
        : []
);

const val = (key: string): unknown =>
    props.activeField
        ? (props.activeField as unknown as Record<string, unknown>)[key]
        : undefined;
</script>

<template>
    <div
        class="hidden lg:flex w-64 bg-white border-l border-gray-300 flex-col overflow-y-auto"
    >
        <div class="p-4">
            ...
            <template v-else>
                <div class="text-center py-10">
                    ...
                    <p class="text-xs text-gray-400">
                        Click a field on the left or select an inline chip in
                        the document to edit its properties.
                    </p>
                </div>
            </template>
        </div>
