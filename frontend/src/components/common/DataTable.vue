...
<template>
  <div>
    ...
    <div v-else-if="items.length === 0"
      ...
      <slot name="empty">
        <p class="text-sm text-gray-500">{{ emptyText ?? 'No records found.' }}</p>
      </slot>
    </div>

    <template v-else>
      <!-- Desktop Table -->
      <div class="hidden lg:block bg-white border border-gray-300 rounded-lg overflow-hidden shadow-sm">
        <table class="w-full">
          ...
          >
            ...
            >
              <slot :index="index" :item="item" :name="`cell:${header.key}`">
                {{ item[header.key] }}
              </slot>
            </td>
          </tr>
          ...
          <slot :index="index" :item="item" name="mobile-item">
            <div class="space-y-1.5">
              ...
              >
                ...
                <span class="text-gray-900 font-medium text-right">{{ item[header.key] }}</span>
              </div>
            </div>
          </slot>
        </div>
