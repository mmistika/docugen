...
watch(() => [props.show, props.member] as const, ([show, member]) => {
  if (!show) return
    ...
    <template #body>
      <div class="space-y-2">
        <p class="text-xs text-gray-500 mb-3">
          Select all roles this member should have. Changes replace the current assignment.
        </p>
        <label
            v-for="role in availableRoles"
            :key="role"
            class="flex items-center gap-3 px-3 py-2.5 rounded border cursor-pointer
                 transition-colors hover:bg-gray-50"
            :class="selected.has(role)
            ? 'border-gray-900 bg-gray-50'
            : 'border-gray-200'"
        >
          <div
              class="w-4 h-4 rounded border flex items-center justify-center shrink-0 transition-colors"
              :class="selected.has(role) ? 'bg-gray-900 border-gray-900' : 'border-gray-300'"
              @click.prevent="toggle(role)"
          >
            <svg v-if="selected.has(role)" class="w-2.5 h-2.5 text-white" fill="none" viewBox="0 0 10 8">
              <path d="M1 4l3 3 5-6" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
          </div>
          <input type="checkbox" :checked="selected.has(role)" @change="toggle(role)" class="sr-only" />
          <span class="text-sm text-gray-800 font-medium">{{ role }}</span>
        </label>
